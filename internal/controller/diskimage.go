...
// downloadContextTimeout is the maximum time allowed for downloading a DiskImage (ISO + firmware).
const downloadContextTimeout = 30 * time.Minute
...
func (c *Controller) downloadDiskImage(parentCtx context.Context, di *k8s.DiskImage) {
	...
	downloadCtx, cancel := context.WithTimeout(parentCtx, downloadContextTimeout)
	defer cancel()
...
func (c *Controller) downloadAndVerify(ctx context.Context, fileURL, destPath string) (*k8s.DiskImageVerification, error) {
	...
	// Create parent directory with restricted permissions (owner and group only)
