...
func (c *Controller) checkBootTargetReady(ctx context.Context, provision *k8s.Provision) (bool, string) {
	...
	bootMedia, err := c.k8sClient.GetBootMedia(ctx, bootTarget.BootMediaRef)
	if err != nil {
		return false, fmt.Sprintf("BootMedia '%s' not found (referenced by BootTarget '%s')", bootTarget.BootMediaRef, bootTarget.Name)
	}
...
func (c *Controller) validateProvisionRefs(ctx context.Context, provision *k8s.Provision) error {
	...
	bt, err := c.k8sClient.GetBootTarget(ctx, provision.Spec.BootTargetRef)
	if err != nil {
		return fmt.Errorf("BootTarget '%s' not found", provision.Spec.BootTargetRef)
	}
	if _, err := c.k8sClient.GetBootMedia(ctx, bt.BootMediaRef); err != nil {
		return fmt.Errorf("BootMedia '%s' not found (referenced by BootTarget '%s')", bt.BootMediaRef, bt.Name)
	}
